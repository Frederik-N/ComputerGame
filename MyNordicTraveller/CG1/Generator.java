	...
	private static Game generateGame(int seed, String[] data) {
		...
		for(String line : data) {
			if(line.startsWith("\t") && roads) {
				...
				if(country == null) {
					throw new NetworkParseException("No country defined.");
				}
				String[] city = line.substring(1).replace(" ","").split(",");
				int value = Integer.parseInt(city[1]);
				City c = new City(city[0].replace("#", "").replace("|", ""), value, country);
...
/**
 * This exception is thrown if there was a problem parsing the network.dat file.
 * @author Nikolaj Ignatieff Schwartzbach
 * @version 1.0.0
 */
